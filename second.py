...
def str_replace(text: str, ch: str) -> str:
    result = ""
...
print(str_replace(text, ch))  # DataCampBlackFridaySale
...
def can_segment_str(s: str, dictionary: list) -> bool:
...
print(can_segment_str(s, dictionary))  # True
...
def removeDuplicates(array: list) -> int:
    ...
    for i in range(1, len(array)):
        if array[i - 1] != array[i]:
            array[insertedIndex] = array[i]
            insertedIndex += 1
    return insertedIndex


array_1 = [1, 2, 2, 3, 3, 4]
print(removeDuplicates(array_1))  # 4


array_2 = [1, 1, 3, 4, 5, 6, 6]
print(removeDuplicates(array_2))  # 5
...
def buy_sell_stock_prices(stock_prices: list) -> tuple:
...
print(buy_sell_stock_prices(stock_prices_1))  # (9, 23)


stock_prices_2 = [8, 6, 5, 4, 3, 2, 1]
print(buy_sell_stock_prices(stock_prices_2))  # (6, 5)
...
def find_missing(input_list: list) -> int:
    n = len(input_list) + 1
...
list_1 = [1, 5, 6, 3, 4, 2, 9, 8]


print(find_missing(list_1))  # 2
...
def checkTriplet(array: list) -> bool:
    n: int = len(array)
...
solve_coin_change(denominations, amount)  # 4
...
print(max_index_diff(array_1))  # 6
...
print("Merit" if score < 50 else "Fail" if score < 70 else "Distinction") # Distinction
